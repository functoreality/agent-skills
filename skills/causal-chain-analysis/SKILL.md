...
- 如果是，分析它们是否存在微妙的差异，在两个上下文中实际指向不同的因素，例如对应不同的对象/时间/发生条件等
- 如果实际上不同，建议修改表述以区分，例如添加限定词（定语/状语等）
- 如果确实相同，不需要重复展开，在该节点行末标注 `：上方已分析` 或 `：下方已分析`，"上方/下方"按文件行号判断（已有节点所在行号小于当前则为上方，大于则为下方），而非按缩进层级
- 如果出现的是完全相同因素的反面（如当前节点"X过大"而已有节点"X过小"），且预判其后续展开逻辑完全对应已有节点的反向、不存在任何特殊之处，同理标注 `：上/下方已分析其反面`
