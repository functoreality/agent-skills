...
- 如果出现过的相同表述存在实质上不同，考虑修改因素表述、补充状语，以进行区分
...
**3. 重读规范**：你自己重新使用 read 工具读一遍本 SKILL.md 以及 references/ 下的辅助文档。
...
1. X 是 Y 的唯一直接子节点
2. Y 也是其父节点的唯一直接子节点。Y 若无缩进（如初始缺点），视为虚拟根的直接子节点，此时要求 Y 是根的唯一子节点
