...
只要队列非空，重复以下流程：从队列中取出优先级最高的一个节点，对它执行展开。一次完整的展开包含以下子步骤。
...
因果链持续膨胀，建议定期启动一个子 agent 扫描分析文件。
频率：每新增约 20-30 节点，或每完成一个主要分支后。
它只提建议、不修改文件。

让它加载本 SKILL.md 全文以了解所有规范要求，在此基础上额外关注以下 SKILL.md 未专门覆盖的结构性检查：
...
也可直接运行 `scripts/check-leaves <file>` 快速查找待展开节点。

此外，在中期扫描时还应检查因果链中是否有缩进层级过深、内容特别复杂的子树。
...
2. 子 agent 改进结束后，根据它修改后的新版因果链分析文档，重新执行一轮步骤5的 6 项检查，发现新问题就接着修改。再也找不出更多问题后，可以结束分析。
