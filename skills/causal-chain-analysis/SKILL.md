...
本步骤为当前节点产出一组**正确的直接子因素**。最终目的，是把初始缺点的产生条件展开为一套有助于后续发现尽可能多解决办法和干预方向的因果结构。
直接性使每层对应紧邻的发生机制，避免跳过可干预环节。完备性避免遗漏独立致因路径和必要条件。下述拆分依据的优先级通常更能保留机制差异。
这三项均为服务该目的的一般经验，应结合当前节点判断，不能机械套用。

一次展开只选择一种依据产出当前节点的下一层子因素。它把选择依据、确定组合关系、验证三件事放在一个紧闭环里反复迭代，直到验证全部通过，或穷尽所有拆分方式仍失败（此时进入 4.4）。
...
只找最直接的原因（比如由物理上直接接触的组件引起），而非间接原因。当前层只产出父节点的直接子因素，不得顺着某个子因素继续下拆。

**4.3.2 选择拆分依据并产出候选子因素**

为当前节点只选择一种拆分依据，按它产出候选子因素。默认按以下顺序选择。
这个优先级与直接性、完备性一样，都是服务 4.3 最终目的的一般经验，应结合当前节点判断，不要把它当作不可变的 if / elif 规则。

1. **形态完备性枚举**：当父节点可以以多种形态发生，且这些形态的发生机制、后续归因方向或可干预方向不同，先枚举这些形态
	- 例如，"钢球解体"直接原因不是"机械应力大于钢球材料强度"，而应为"钢球受冲击表面断裂 or 钢球内部断裂"。后者再展开时，直接原因可为"钢球内部横向断裂 or 钢球内部纵向断裂"，再后者才在未来归因为"钢球内部纵向机械应力大于钢球材料强度"
...
3. **是否完备**：按 4.3.3 确定的组合关系选择对应的检验逻辑（见"前置知识"）。
	...
	- 如果 4.3.2 没有用公式拆分，或即使用了仍不确定是否完备，可以回头用公式（严格公式或直观公式）交叉检查父节点。公式是避免遗漏的强大手段，但不得把公式项与当前依据产生的子因素混列为同一层
	- 完备性不足则补充后重新验证
