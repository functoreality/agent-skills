...
本步骤为当前节点产出一组**正确的直接子因素**。它把选择依据、确定组合关系、验证三件事放在一个紧闭环里反复迭代，直到验证全部通过，或穷尽所有拆分方式仍失败（此时进入 4.4）。

**4.3.1 目标与基本原则**

最终目的，是在当前分析边界内，把初始缺点的产生条件展开为一套有助于后续发现尽可能多解决办法和干预方向的因果结构。

- **直接性**：每层对应紧邻的发生机制，避免跳过可干预环节
- **完备性**：避免遗漏独立致因路径和必要条件
- **保留机制差异**：把后续归因方向或可干预方向不同的机制分开，使后续分析覆盖更多方向

上述原则和下述拆分依据的优先级，都是服务最终目的的一般经验，应结合当前节点判断，不能机械套用。

一次展开只选择一种依据产出当前节点的下一层子因素。当前层只产出父节点的直接子因素，不得顺着某个子因素继续下拆。
...
这一顺序是 4.3.1 基本原则的一般实现方式，应结合当前节点判断，不要把它当作不可变的 if / elif 规则。
