...
当缺点可以以多种形态发生时，如果它们的机制存在差异，则对这些形态做完备性枚举，以备未来分别归因。
例如，"钢球解体"直接原因不是"机械应力大于钢球材料强度"，而应为"钢球受冲击表面断裂or钢球内部断裂"，后者直接原因"钢球内部横向断裂or钢球内部纵向断裂"，再后者将在未来归因为"钢球内部纵向机械应力大于钢球材料强度"。
这种区分必要，是因为表层应力、内部横向应力、内部纵向应力的后续归因方向不同。
同理，"Transformer注意力计算量大"不应直接用公式 `FLOPs=CLN²d`，应先拆 QKᵀ、softmax、AV 等多模块；这些模块计算机制不同，后续展开优先级也不同
...
1. **试写严格公式**：找出现代科学中对该因素的严格定义或计算公式，把公式中的每一项作为独立分支展开
	...
	- 用直接公式而非整合公式：整合公式符合分解完备性，但不符合直接性，不应使用
		- 如 `F=ma` 是直接公式，而合并 `m=ρV` 后所得 `F=ρVa` 为整合公式，用后者拆出的原因不属于直接原因
		- Transformer注意力计算量 `FLOPs=CLN²d` 也是整合公式（多模块之和）
