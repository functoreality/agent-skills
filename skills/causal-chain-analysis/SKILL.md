...
- 所有新增节点必须来自当前节点的公式拆分或直接分解，不得基于外部参考材料中的论点或其他动机主动添加
	- 若某概念无法从当前节点的任何拆分方式中自然得出，应检查已有分析是否有遗漏或错误
	- 若仍未找合适位置，不应强行加入
...
**2. 子 agent 扫描**：完成子树重构后，启动一个子 agent 扫描分析文件。
它只提建议、不修改文件。
...
**检查方法**：对于每个标注了 `：末端` 的节点，或没有标注但实际上是叶子节点的节点，问自己"这个原因还能继续追问'它发生的机制是什么'吗？"
