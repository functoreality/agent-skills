...
1. 询问"这个表层缺点会导致什么？"
2. 推演它的结果
3. 继续追问，直到找到用户根本上想解决的问题，即本质性的初始缺点
...
**重要**：
- 表述规范：请阅读 `references/notation_specification.md`，对照其中的要求检查初始缺点的表述是否规范，如有问题立即修正
...
2. 在内存中维护一个待展开节点队列，初始时只包含根节点（初始缺点）
3. 每个节点附带一个优先级评估：该节点因素的消除对解决初始缺点的贡献程度。贡献越大、优先级越高（参考"分析过程中的注意事项"中的"二八定律"）。初始缺点本身优先级最高
...
对照 `references/notation_specification.md`，检查当前节点是否符合主语+谓语、明确具体、原子化、客观事实的要求。
...
**定期落盘**：展开过程中，每当新增一定量的分析结果（建议每展开 3 到 5 个节点，或完成一个子树后），把新内容写入文件，避免遗忘。详见"写入文件的时机"一节。
...
让它阅读本 SKILL.md（给完整路径）以了解所有规范要求，在此基础上额外关注以下 SKILL.md 未专门覆盖的结构性检查：
...
虽然每次展开时已经做过表述、跳步、遗漏的即时检查，仍然需要把以下 6 项检查从全局视角完整复盘一遍。因为逐节点检查可能发现不了跨节点、跨子树的系统性问题。

**重要**：对每一项检查，如果发现错误就立即修改文件，不能等 6 项都检查完才一起修改。

**工具使用**：到这一步，因果链分析文件通常已经较长，且使用 tab 缩进表示层级，逐行通读不易把握整体结构。

如果环境中有 outline-read skill 可用，加载它来读取已编写的分析文件，以便按层级折叠、展开与跳转，辅助在 6 项检查中快速定位和浏览各节点。

#### 检查一：每个缺点因素的表述是否满足要求

复查全文档，每个缺点因素必须是**主语+谓语形式的，明确具体的，原子化的客观事实**。详细标准见 `references/notation_specification.md`。

#### 检查二：是否有跳步

复查全文档，逐节点检查每个因果关系是否符合"直接原因"标准（标准与示例见步骤 4.3.4 验证 2）。重点关注逐节点检查时不易发现的跨节点、跨子树逻辑跳跃，发现问题立即补上中间层。

#### 检查三：是否有遗漏因素

复查全文档，是否有步骤 4.3.4 验证 3 中遗漏的因素。重点关注单个节点展开时局部看似完备、但从全局角度看（结合其他子树的信息交叉验证）才发现缺少的必要前提。发现遗漏则补充到相应层级下。

#### 检查四：是否提前终止

检查某个因素节点在之前的分析中是否过早终止，即缩进层级到此为止，但其实现在看看它好像并不是末端缺点，可以继续展开出下一层的原因。
...
**开始前提**：如果你没有完成步骤5，并且确认连续两轮检查都没发现问题，不要进入这一步。
...
1. 创建一个子 agent（需有文件读写能力），让它：
	...
	- 在此基础上额外关注中期扫描所列的结构性检查（缩进、孤儿节点、未展开叶子、过早终端、重复节点、缺失桥接公式）
	- 直接编辑文件进行修改改进
	- 命令它**绝对不要再创建子 agent**

**目的**：通过独立的、上下文无污染的子 agent 审查，发现主 agent 可能遗漏的问题，进一步提升因果链分析的质量。
步骤 6 的 agent 与步骤 4 中期扫描 agent 的区别：前者直接动手修改，后者只提建议。
...
3. 子 agent 改进结束后，根据最终版因果链分析文档，重新执行一轮步骤5的 6 项检查，发现新问题就接着修改。再也找不出更多问题后，可以结束分析。

---

## 表述规范

详细的表述规范（主语+谓语形式、明确具体、原子化、客观事实、必要时补充状语、检查方法、示例对比）见 `references/notation_specification.md`。

何时查阅：

- 步骤4.1：每次展开时即时确认当前节点表述
- 步骤5检查一：收尾复查全文档每个缺点因素的表述
...
**结构示例**：完整的因果链输出样例见 `examples/static_electricity_example.md`，展示了 tab 缩进表示层级、父子与同级关系的实际写法。
...
2. **迭代编辑**：步骤4 展开过程中，每新增一定量分析结果（建议每展开 3 到 5 个节点，或完成一个子树后），把新内容写入文件，避免后续遗忘
...
3. 初始化文件与队列：写入"手指感到疼痛"作为根，加入队列（优先级最高）
4. 逐节点展开（示意）：
	- 展开"手指感到疼痛"：
		- 确认表述 OK，不重复，尝试拆分
...
4. 进入逐节点展开的核心循环，每次取优先级最高的节点展开，即时确认表述、检查重复、尝试拆分、检验拆分、判断末端、登记子节点，定期写入文件（步骤4）
