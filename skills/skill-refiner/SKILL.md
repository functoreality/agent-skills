...
> 注意：`learnings.md` 不在本阶段读取，仅在阶段四出现可升级类弱观测时按 §4.4 交叉验证。
...
当某个观测被判定为"证据不足以直接写入 skill"、且属于可能升级为 skill 规则的类别（泛化失败、待决改动）时，在决定其去向前，先查阅目标 skill 的 learnings.md，检索是否有同模式先例：

- **有先例**：升级本次观测，它不再是孤例。按改动建议层级呈现改动方案（引用先例作为佐证），并在方案中标注 learnings.md 中对应旧条目将在执行阶段删除（见阶段六）。
- **无先例**：维持弱观测处理，按 §4.3 待决改动层级呈现两选项，其中"记入 learnings.md"即作为新观测追加。
...
3. 如果此次分析产生了新的、尚未达到"改动建议"级别的观察（如 §4.4 交叉验证无先例、泛化失败、模型能力限制），追加到 learnings.md
...
| 强度 | 条件 | 处理 |
|---|---|---|
| **弱** | 单次观察，learnings.md 中亦无同模式先例 | 经 §4.4 交叉验证无先例后，按 §4.3 待决改动层级呈现两选项，由用户决定直接改 skill 还是记入 learnings |
| **中** | 同模式在 ≥2 次独立执行中出现（含 learnings.md 中的先例），或已通过初步跨领域泛化检验 | 提议为改动建议 |
