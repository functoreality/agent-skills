...
**注意**：一行修改可能同时包含多个独立考量（如改了一个表述、换了一个词序、拆了行——三者各自对应不同的动机）。逐考量分别归因，不要笼统归为一条。
...
每个改动按其规模/风险/自信度归入三个层级之一；无论哪一层都输出 skill 改动 diff 供用户决策：

| 层级 | 条件 | 做法 |
|---|---|---|
| **候选补丁**（Candidate Patch） | 改动很小、副作用风险极低 | 输出简洁改动方案供用户确认 |
| **改动建议**（Proposed Change） | 改动较显著或有争议 | 输出改动方案 + 推演验证供用户审阅 |
| **待决改动**（Pending Decision） | 不确定是否应固化为 skill 规则 | 同时输出「若固化的简洁改动方案」和「记 learnings.md 的摘要」二选项，由用户选择；若选 learnings，将改动方案一并记录在该条目中，便于未来直接采用 |

**每一层改动方案都应包含**：
- skill 改动计划
  - 优先用 diff 格式（`-` 旧行 / `+` 新行 / ` ` 共享上下文）
