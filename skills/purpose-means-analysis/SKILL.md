...
用户最先提出的功能不一定是本次分析的根。按以下过程逐层上溯：

1. 问“实现这个需求，直接服务什么正向功能、输出或目标状态？”
2. 将答案视为上一层目的，继续追问。每上溯一层，检查它是否会打开
   与原需求不同的实现方向
3. 直到再上溯只能得到脱离当前项目的空泛价值判断，或不再改变方案空间时停止。
   不把“让用户更方便”“改善生活”这类抽象价值判断写成根本目的
4. 不确定当前层是否已足以作为根本目的时，先向用户确认

例如，用户提出“按距离排序”时，可推得它服务于“使用户预期或缩短送达时间”，
而非直接把排序功能作为根。此时按距离排序与 15 分钟配送专区可成为不同手段；
若再上溯到“满足日常饮食需要”已脱离当前配送项目的方案空间，即可停止。
...
已知当前无法取得的资源对应手段也不删除，可标为
`：暂缓（资源当前不可得）`，不继续展开；是否采用留给分析结果的后续使用决定。
