...
2. 识别用户真正希望实现的正向功能、输出或对象，
   而非过早锁定某一既有方案
3. 用户给出具体方法或局部操作时，先判断其直接服务的正向功能
   是否已足以确定本次边界；仅在上层目的会改变边界时，追问它服务什么目的
4. 将根本目的改写为符合节点规范、清晰、具体、可继续展开的表述
5. 若用户给出的是已有系统或缺点，区分两种任务：
   - 若要解释或修补原系统内的缺点，本 Skill 不适用
   - 若要用新系统实现原本应有的功能，将该功能作为根本目的
6. 根本目的不必预先附带资源、成本或一般可行性限制。
   明确不可违反的范围、法律、安全或接口约束，应在分析约定中说明，
   不展开与其冲突的分支；其他信息通常属于具体手段的实施条件，
   或用于调整展开优先级
...
- 同一子问题在不同位置重复出现时，标注 `：见「首次出现的节点或独立子树」`
...
1. **直接性**：每个子节点都应直接服务父节点。
   ...
   若某节点只是在远处可能有帮助，
   先补出它直接服务的中间子目的。
2. **And 局部完备性**：在当前声明的范围和假设中，假设已列子节点全部完成，
   问父目的是否仍可能无法实现。如果可能，
...
若审查发现实质问题，主分析者修正后可继续进行下一轮独立审查，
直到一轮不再发现未解决的高严重度问题，或独立审查累计达到 3 次。
达到上限后仍有实质问题时，须在交付中说明。
